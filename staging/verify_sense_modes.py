...
class FakeSense(PixelOnlySense):
    def __init__(self, hardware_rotation=False):
        super().__init__()
        self.rotations = []
        self.hardware_rotation = hardware_rotation
    def set_rotation(self, rotation):
        if not self.hardware_rotation:
            raise AttributeError('fallback display has no hardware rotation')
...
def draw_mode(mode, sense, st, tick=3):
    if mode in ('compass', 'compass-arrow'):
        ...
        mod.draw_rotation_test(sense, st, tick)
    elif mode == 'liquid':
        mod.draw_liquid(sense, orient, tick, st)
    elif mode == 'pacman':
        mod.PACMAN_STATE = {}
        for t in range(24):
        ...
        mod.draw_fire(sense, tick, st)
    elif mode == 'flashlight':
        mod.draw_flashlight(sense, st)
    elif mode == 'sos':
        mod.draw_sos(sense, tick, st)
    elif mode == 'message':
        mod.text_once(sense, st['sense_message'], (0, 150, 80), 0.055, st)
    elif mode == 'boot':
        mod.text_once(sense, 'WELCOME TO THE WILDERNESS', (0, 120, 20), 0.055, st)
    elif mode == 'sun':
        mod.draw_sun(sense, {**st, 'sun_message': 'SUN TEST'})
    elif mode == 'gps':
        mod.draw_gps(sense, {'mode': 3, 'satellites': 6}, st)
    elif mode == 'clock':
        mod.text_once(sense, '12:34 PM', (80, 80, 180), 0.055, st)
    elif mode == 'progress':
        mod.draw_route_progress(sense, st)
    elif mode == 'beacon':
        mod.sense_clear(sense, 0, 80, 120, st)
    elif mode == 'stars':
        ...
        mod.draw_bar(sense, 32, (0, 120, 0), st)
    elif mode == 'humidity':
        mod.draw_bar(sense, 44, (0, 60, 180), st)
    elif mode == 'pressure':
        mod.draw_bar(sense, 38, (120, 80, 180), st)
    elif mode == 'avatar':
        ...
        mod.draw_border(sense, (0, 170, 85), st)
    elif mode == 'magic8':
        mod.text_once(sense, 'YES', (80, 0, 180), 0.055, st)
    elif mode == 'water':
        mod.draw_water_icon(sense, (0, 90, 220), st)
    elif mode == 'snake':
        mod.draw_snake_frame(sense, tick, (0, 220, 70), st)
    else:
...
# Fallback/simulator orientation: displays without set_rotation must rotate pixels manually.
pattern = [[0, 0, 0] for _ in range(64)]
pattern[0] = [255, 0, 0]
pattern[1] = [0, 255, 0]
for rotation, expected_lit in [(0, {0, 1}), (90, {7, 15}), (180, {62, 63}), (270, {48, 56})]:
    st = {**base_state, 'sense_rotation': rotation}
    sense = PixelOnlySense()
    mod.sense_set_pixels(sense, pattern, st)
    assert lit(sense.frames[-1]) == expected_lit, (rotation, lit(sense.frames[-1]))

# Real Sense HAT orientation: hardware set_rotation is the single source of truth;
# pixels/text/clear are not double-rotated and all receive the same setting.
for rotation in (0, 90, 180, 270):
    st = {**base_state, 'sense_rotation': rotation}
    sense = FakeSense(hardware_rotation=True)
    mod.sense_set_pixels(sense, pattern, st)
    mod.sense_clear(sense, 1, 2, 3, st)
    mod.text_once(sense, 'ROT', (10, 20, 30), 0.01, st)
    assert sense.rotations[0] == rotation, (rotation, sense.rotations)
...
print('orientation_global=PASS hardware_rotations=(0,90,180,270) fallback_pixel_rotation=PASS')
