#!/usr/bin/env bash
set -euo pipefail
PLUGIN_ID='desert-botanical-theme'
MARKER_DIR="/var/lib/mappi3/plugins/$PLUGIN_ID"
install -d -m 0775 "$MARKER_DIR"
install -d -m 0775 '/var/lib/mappi3/themes/desert-botanical-theme'
cat > "$MARKER_DIR/installed.json" <<JSON
{"id":"$PLUGIN_ID","status":"installed","offline_safe":true,"installed_at":"$(date -Is)","features":["desert_botanical"],"note":"Theme metadata installed; UI theme is bundled in the web app."}
JSON
