...
install -d -m 0775 '/var/lib/mappi3/themes/rainforest-immersive-theme'
cat > "$MARKER_DIR/installed.json" <<JSON
{"id":"$PLUGIN_ID","status":"installed","offline_safe":true,"installed_at":"$(date -Is)","features":["rainforest_immersive"],"note":"Theme metadata installed; UI theme is bundled in the web app."}
JSON
